    ...
    for name in all_accounts:
        if name == login_name:
            ...
            if login_password == password:
                ...
                Label(
                    account_dashboard, text="welcome" + name, font=("Calibri", 12)
...
def deposit():
    ...
    deposit_notif = Label_screen, font = ("Calibri", 12)
...
def finish_deposit():
    ...
    if float(amount.get()) <= 0:
        deposit_notif.config(text="Navigate currency is not accepted", fg="red")
        return
...
def withdraw():
    ...
    # Label
    Label(withdraw_screen, text="Deposit", font=("Calibri", 12)).grid(
        row=0, sticky=N, pady=10
    )
    current_balance_label = Label(
        withdraw_screen,
        text="Current Balance : $" + withdraw_balance,
        font=("Calibri", 12),
    )
    current_balance_label.grid(row=1, sticky=W)
    Label(withdraw_screen, text="Amount : $", font=("Calibri", 12)).grid(
        row=2, sticky=W
    )
    withdraw_notif = Label_screen, font = ("Calibri", 12)
    withdraw_notif.grid(row=4, sticky=N, pady=5)
    # Entry
    Entry(withdraw_screen, textvariable=amount).grid(row=2, column=1)
    # Button
    Button(
        withdraw_screen, text="Finish", font=("Calibri", 12), command=finish_deposit
    ).grid(row=3, sticky=W, pady=5)
...
def finish_withdraw():
    ...
    if float(withdraw_amount.get()) <= 0:
        withdraw_notif.config(text="Navigate currency is not accepted", fg="red")
        return
...
# Image Import
img = Image.open("1_bank.jpg")
img = img.resize((150, 150))
img = ImageTk.PhotoImage(img)

# Labels
Label(master, text="Custom Banking Beta", font=("Calibri", 14)).grid(
    row=0, sticky=N, pady=10
)
Label(
    master, text="the most secure bank you´ve probably used", font=("Calibri", 12)
).grid(row=1, sticky=N)
Label(master, image=img).grid(row=2, sticky=N, pady=15)

# Buttons
Button(master, text="Register", font=("Calibri", 12), width=20, command=register).grid(
    row=3, sticky=N
)
Button(master, text="Login", font=("Calibri", 12), width=20, command=login).grid(
    row=4, sticky=N, pady=20
)
